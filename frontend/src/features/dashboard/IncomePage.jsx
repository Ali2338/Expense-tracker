...
import { PlusCircle, Trash2, ArrowUpRight, Calendar, Tag, CreditCard } from 'lucide-react';
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip } from 'recharts';

function IncomePage({ incomeRecords, incomeChartData, onLogTransaction, onDeleteTransaction }) {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    onLogTransaction({ title, amount, category, date, type: 'INCOME' });
    setTitle(''); setAmount(''); setCategory('Salary'); setDate('');
  };

  return (
    <div className="space-y-6 animate-fadeIn w-full box-border">
      <div>
        <h2 className="text-2xl font-black tracking-tight text-emerald-600 flex items-center gap-2">
          Income Portal <ArrowUpRight size={24} />
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start w-full">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-xs w-full box-border">
          <h3 className="text-sm font-bold mb-4 text-slate-900 dark:text-white">Add Income</h3>
          <form onSubmit={submitForm} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Freelance Consulting" className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none text-slate-900 dark:text-white focus:border-emerald-500 transition-colors" required />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select value={category} onChange={e => setCategory(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none text-slate-900 dark:text-white focus:border-emerald-500 transition-colors cursor-pointer" required>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none text-slate-900 dark:text-white focus:border-emerald-500 transition-colors" required />
              </div>
            </div>

            <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-lg shadow-emerald-600/10 active:scale-98 transition-all"><PlusCircle size={14} /> Add Transaction</button>
          </form>
        </div>

        {/* Chart Viewport & History Timeline Panel */}
        <div className="lg:col-span-2 space-y-6 w-full box-border">
          <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-xs w-full overflow-hidden">
            <h3 className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-4">Income Sources</h3>
          ...
          <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-xs max-h-[350px] overflow-y-auto space-y-2 w-full box-border">
            <h3 className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-2">Recent Income</h3>
            {incomeRecords.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-8">No income transactions logged yet.</p>
            ) : (
              incomeRecords.map(item => (
                <div key={item.id} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-100/70 dark:border-slate-800 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-950/80 transition-all group gap-2">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <div className="p-2 bg-emerald-100/50 dark:bg-emerald-950/40 text-emerald-600 rounded-lg text-xs font-bold shrink-0">+₹</div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                    <span className="text-emerald-600 font-extrabold text-xs sm:text-sm whitespace-nowrap">
                      +₹{parseFloat(item.amount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                    <button onClick={() => onDeleteTransaction(item.id)} className="text-slate-300 hover:text-rose-500 cursor-pointer p-1 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
